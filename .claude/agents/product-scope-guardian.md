...
- The product must create a readable repository lore at a stable URL. The MVP does not include scheduled or automatic refresh — analysis runs on submission and on manual re-analysis only.
- The MVP does not include health scores or generalized health findings.
...
- AI must explain evidence-backed analysis; it must not replace deterministic code analysis, and it is not required for the MVP.
