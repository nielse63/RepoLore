...
- Some path-aliased imports may not resolve to a file without additional (non-type-checker) alias-resolution logic reading `tsconfig.json` `paths` directly; this is scoped into source file discovery/config reading (implementation session 6), not the type checker. **Revision (see ADR-0015):** implemented as a non-type-checker addition to `module-resolution.ts`/a new `project-config.ts`, exactly as anticipated here — not a rewrite of the syntactic-only approach — and expanded beyond `tsconfig.json` alone to `jsconfig.json`, `package.json`'s `imports` field, and several bundler/framework configs.
