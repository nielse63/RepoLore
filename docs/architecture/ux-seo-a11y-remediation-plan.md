...
- **Row selection (RL-003)**: the first cell's icon + name wraps in a real, in-flow `<button>` with an accessible name (`View {name} dependency details`) — a genuine, singular interactive element rather than a `tabIndex` hack on the `<tr>`, so keyboard users can reach and activate it. The `<tr>` keeps its own `onClick` as a pointer-only convenience for the rest of the row (not yet reachable by keyboard outside the first cell).
...
- Dependency row: a real `<button aria-label="View {name} dependency details">` wrapping the icon + name in the first cell (RL-003, fixed).
...
- **`/lore/{owner}/{repo}/search`** — a preview scaffold (`"use client"`, hardcoded fixture data via `PreviewBanner`'s "Search isn't implemented yet for this repository", not linked from `Sidebar`'s `NAV_ITEMS`) that this pass's audit review missed entirely. Its `SearchInput` (RL-006) and `Tabs` (RL-002) instances got the same fixes as `DependenciesContent`'s, since those were cheap and identical in shape. Left unfixed: it still has no `generateMetadata`/`noindex` of its own. It no longer inherits a wrong canonical from the root layout (`alternates.canonical` moved off the layout onto `src/app/page.tsx` — see below), so it's back to having no canonical at all, matching its pre-PR state, but it's still indexable by default and has no route-specific title/description/OG/Twitter. Fixing that properly means restructuring the file into a server page (`generateMetadata`) wrapping a client content component, matching the five real lore routes and the home page (`src/components/HomeContent.tsx`) — real work on a route with no real content yet. Flagged rather than fixed; revisit once the route either gets real search results or gets removed. **Resolved 2026-09-21: removed.** The route, its `SearchInput`/`Tabs` usage, and the `payments-service.ts`/`icons.ts` fixtures it alone depended on are all gone — see `docs/architecture/implementation-plan.md`'s 2026-09-21 entry.
- **Root-layout metadata inheritance** — a layout-level `alternates.canonical` is inherited by every route that doesn't declare its own, which wrongly pointed `/lore/{owner}/{repo}/search` and `not-found.tsx` at the home page. Fixed by moving `alternates: { canonical: "/" }` off `src/app/layout.tsx` and onto `src/app/page.tsx` (now a Server Component wrapping the client `HomeContent`). Title/description/Open Graph/Twitter stay on the root layout as shared defaults, since only the canonical assertion was actively wrong for other routes.
