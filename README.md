...
- **Real, backed by actual analysis:** the overview page (`/lore/{owner}/{repo}`, `repository-overview.png`), the home page (`/`, `home.png`), and the architecture page (`/lore/{owner}/{repo}/architecture`, `architecture.png`). Architecture renders only what the `Lore` model already produces — Major Areas, the direct relationships between them, Entry Points, and each project's `frameworks` — as evidence-backed lists/tables, per **ADR-0008**: no diagram, no infrastructure/data-store detection, no framework detection beyond what the analyzers already support, and no "architectural boundaries" narrative claims, since none of those are backed by real evidence today. See `docs/architecture/decisions/0008-architecture-page-real-data-scope.md` for the full reasoning.
